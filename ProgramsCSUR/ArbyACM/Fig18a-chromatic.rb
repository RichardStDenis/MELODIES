...
module Main
  ...
  g_instance = GraphColoring.get_graph('/home/ubuntu/ArbyACM/'+str+'.json')

  print "Enter a file name for a coloring (without extension): "
  str = STDIN.gets.strip
  c_instance = GraphColoring.get_partial_coloring('/home/ubuntu/ArbyACM/'+str+'.json',
  ...
  while sol.satisfiable? do
    ...
    n = c_instance.colors.size
    if pi_clr.size == n then break end
    ...
    bnds = g_instance.set_bounds(bnds)
    bnds = c_instance.set_bounds(bnds, g_instance.vertices.atoms)
    sol = GraphColoring.solve(:colored, bnds) end
  print "The chromatic number: "; puts n end
