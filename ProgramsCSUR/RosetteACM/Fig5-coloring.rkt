...
(let* ([bnds (bounds U
       ...
       [cntrex (verify #:assume (assert (interpret* (and graph-a1 graph-a2) intpn))
                       #:guarantee (assert (interpret* Undirected intpn)) )] )
  (cond [(sat? cntrex) (printf "The graph is not undirected.\n")]) )
; Query for solving the proper vertex coloring problem (with bound setting)
(let* ([bnds (bounds U
         (list (make-exact-bound Vertex tVertex)
               (make-exact-bound Edge tEdge)
